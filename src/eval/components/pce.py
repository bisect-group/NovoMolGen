...
Adapted from https://github.com/aspuru-guzik-group/Tartarus/blob/main/tartarus/pce.py
"""
...
from src.eval.utils import run_command  # noqa
...
def get_pce(smile, verbose=False, scratch: str = "/tmp"):
    ...
    command_pre = "CHARGE={};xtb {} --gfn 0 --opt normal -c $CHARGE --iterations 4000".format(
        charge, "test.xyz"
    )
    system(command_pre)
    ...
    command_crest = "CHARGE={};crest {} -gff -mquick -chrg $CHARGE --noreftopo".format(
        charge, "xtbopt.xyz"
    )
    system(command_crest)
    system("rm ./gfnff_charges ./gfnff_topo")
    system(f"head -n {atom_number + 2} crest_conformers.xyz > crest_best.xyz")

    # Run the calculation:
    command = "CHARGE={};xtb {} --opt normal -c $CHARGE --iterations 4000 > out_dump".format(
        charge, "crest_best.xyz"
    )
    system(command)

    # Read the output:
    with open("./out_dump") as f:
    ...
    mol_dipole = [
        text_content[i : i + 4] for i, x in enumerate(text_content) if "molecular dipole:" in x
    ]
    ...
    _ = mol_dipole_val + HL_range_rest - lumo_val  # Maximize this function
...
def get_fingerprint(smile, nBits, ecfp_degree=2):
    m1 = MolFromSmiles(smile)
    fp = GetMorganFingerprintAsBitVect(m1, ecfp_degree, nBits=nBits)
...
if __name__ == "__main__":
    ...
    smi = "c1ccccc1"
    pce_pcbm_sas, pce_pcdtbt_sas = get_pce(smi)
