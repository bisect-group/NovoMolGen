...
from rdkit.Chem.Lipinski import NumHAcceptors, NumHDonors  # type: ignore

# Set up the project root
rootutils.setup_root(__file__, indicator=".project-root", pythonpath=True)

from src.eval.components.moses import SA  # noqa
from src.eval.utils import disable_rdkit_log  # noqa
...
inf = open(os.path.join("./data/filters", "pains.txt"))
...
def substructure_violations(mol):
    ...
    for ni in range(len(forbidden_fragments)):
        if mol.HasSubstructMatch(rdc.MolFromSmarts(forbidden_fragments[ni])) is True:
...
def filter_by_pattern(mol, pattern):
    ...
    if mol.HasSubstructMatch(rdc.MolFromSmarts(pattern)) is True:
        violation = True
...
def filter_phosphorus(mol):
    ...
    if mol.HasSubstructMatch(rdc.MolFromSmarts("[P,p]")) is True:
        if mol.HasSubstructMatch(rdc.MolFromSmarts("*~[P,p](=O)~*")) is False:
            violation = True
...
def apply_filters(smi):
    try:
        ...
        # Filter by proper phosphorus
            ...
            return False
        elif Descriptors.NumRotatableBonds(mol) > 10:  # type: ignore
            return False
        elif Descriptors.TPSA(mol) > 140:  # type: ignore
            return False
        elif passes_wehi_mcf(smi) is False:
            return False
...
def is_good_molecule(smiles):
    ...
    if sa_score < 5 and qed_score > 0.5:
        return True
    else:
        return False
