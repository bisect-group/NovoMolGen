...
Adapted from https://github.com/aspuru-guzik-group/Tartarus/blob/main/tartarus/tadf.py
"""
...
from src.eval.utils import run_command, suppress_output  # noqa
...
class molecule:
    ...
    def generate_coordinates_rdkit(self):
        ...
        try:
            ...
            print(
                "ERROR: could not calculate 3D coordinates from rdkit molecule %s." % (self.smiles)
            )
            exit()
        try:
            MMFFOptimizeMolecule(self.mol)
        except Exception:
            print(
                "ERROR: could not optimize 3D coordinates for rdkit molecule %s." % (self.smiles)
            )
            exit()
        try:
            CanonicalizeMol(self.mol, normalizeCovar=True, ignoreHs=False)
        except Exception:
            print(
                "ERROR: could not canonicalize 3D coordinates for rdkit molecule %s."
                % (self.smiles)
            )
            exit()
        try:
    ...
    def generate_coordinates_obabel(self):
        ...
        try:
            ...
            print(
                "ERROR: could not calculate 3D coordinates from openbabel molecule %s."
                % (self.smiles)
            )
            exit()
        try:
            ...
            print(
                "ERROR: could not read xyz coordinates from openbabel molecule %s." % (self.smiles)
            )
            exit()
        return
    ...
    def generate_coordinates_from_file(self):
        ...
        cwd = pl.Path.cwd()
        try:
            with open(pl.Path(cwd / self.file)) as f:
                block = f.read()
                blocklines = block.split("\n")
                self.xyz = []
                self.atoms = []
            ...
            print(
                "ERROR: could not generate xyz coordinates for molecule %s from file. Exiting."
                % (self.smiles)
            )
            exit()
        return
    ...
    def generate_coordinates(self):
        ...
        # Generate XYZ from SMILES if no file specified
            ...
            try:
                ...
                sp.run(
                    ["mv", self.file, "obabel.xyz"],
                    stdout=sp.DEVNULL,
                    stderr=sp.DEVNULL,
                )
            except Exception:
                energies.append(10000.0)
            try:
            ...
            if energies[0] == 10000.0 and energies[1] == 10000.0:
                print(
                    "ERROR: could not generate xyz coordinates for molecule %s from file. Exiting."
                    % (self.smiles)
                )
...
class computation:
    ...
    def pre_xtb(self):
        ...
        sp.run(
            [xtb, self.file, "--gfn", "0", "--opt", "normal", "--iterations", "4000"],
            stdout=sp.DEVNULL,
            stderr=sp.DEVNULL,
        )
        return
    ...
    def xtb(self):
        ...
        with open("crest_best.xyz", "w") as file:
            sp.run(
                ["head", "-n", str(self.atom_count + 2), "crest_conformers.xyz"],
                stdout=file,
                stderr=sp.DEVNULL,
            )
    ...
    def pyscf(self):
        with suppress_output(self.verbose):
            ...
            self.method.xc = "B3LYP"  # type: ignore
            self.method.max_cycle = 512  # type: ignore
            self.method.grids.level = 0  # type: ignore
            self.method.conv_tol = 1e-7  # type: ignore
            self.method.kernel()  # type: ignore
            ...
            self.oscillator_strength_singlets = self.calculation.oscillator_strength(
                gauge="length"
            )
            # Triplets
            ...
            self.singlet_triplet_gap = min(self.excitation_energies_singlets) - min(
                self.excitation_energies_triplets
            )
            self.oscillator_strength = self.oscillator_strength_singlets[0]
            self.results = np.array(
                [
                    self.fluorescence_energy,
                    self.singlet_triplet_gap,
...
def get_tadf(smi: str, verbose: bool = False, scratch="/tmp"):
    ...
    try:
        ...
        results_ = (
            comp.results
        )  # results_[0]=color; results_[1]=S-T; results_[2]=Oscillator Strength;

        system("rm -rf core.*")
        run_time = time.time() - start_time  # noqa

        if results_[1] <= 0.0 or results_[0] <= 0.0 or results_[2] <= 0.0:
            std, osc, combined = -(10**4), -(10**4), -(10**4)  # noqa
        else:
            st, osc, combined = (
                -results_[1],
                results_[2],
                results_[2] - results_[1] - np.abs(results_[0] - 3.2),
            )
