...
Adapted from https://github.com/aspuru-guzik-group/Tartarus/blob/main/tartarus/reactivity.py
"""
...
from src.eval.components.moses import SA  # noqa

KEYWORDS_CREST = ["-mquick", "-gfn2//gfnff", "-noreftopo"]


def geometry_from_openbabel(mol: Chem.Mol) -> tuple[NDArray, NDArray]:
...
def apply_chiral_template(mol: Chem.Mol, pattern: Chem.Mol) -> Chem.Mol:
    ...
    indices_matched = []
    for bond in pattern.GetBonds():  # type: ignore
    ...
    bond_info = []
    for bond in list(rw_mol.GetBonds()):  # type: ignore
    ...
    for i, atom in enumerate(pattern.GetAtoms()):  # type: ignore
...
def process_smiles(
    smiles: str,
) -> tuple[tuple[Chem.Mol, Chem.Mol, Chem.Mol], tuple[int, int, int], tuple[int, int, int]]:
    ...
    # Do conformer search
    elements = [atom.GetSymbol() for atom in mol.GetAtoms()]  # type: ignore
    coordinates = mol.GetConformer().GetPositions()
    conformer_ensemble = opt_crest(elements, coordinates, keywords=KEYWORDS_CREST)
    mol.RemoveAllConformers()
    _add_conformers_to_mol(mol, [conformer_ensemble[0].coordinates])  # type: ignore
    ...
    indices_h_l = [
        neighbor.GetIdx() for neighbor in atom_c_s_l.GetNeighbors() if neighbor.GetSymbol() == "H"
    ]
    indices_h_r = [
        neighbor.GetIdx() for neighbor in atom_c_s_r.GetNeighbors() if neighbor.GetSymbol() == "H"
    ]
    ...
    for dist_1, dist_2 in zip(
        ...
        ff_properties = MMFFGetMoleculeProperties(mol_prod)
        ff = MMFFGetMoleculeForceField(mol_prod, ff_properties)
        ...
        ff.Initialize()
        ff.Minimize()
    MMFFOptimizeMolecule(mol_prod)

    mol_prod_ts = Chem.Mol(mol_prod)

    # Do conformer search
    elements = [atom.GetSymbol() for atom in mol_prod.GetAtoms()]  # type: ignore
    coordinates = mol_prod.GetConformer().GetPositions()
    conformer_ensemble = opt_crest(elements, coordinates, keywords=KEYWORDS_CREST)
    mol_prod.RemoveAllConformers()
    _add_conformers_to_mol(mol_prod, [conformer_ensemble[0].coordinates])  # type: ignore
...
def barrier_from_smiles(
    smiles: str, n_images: int = 5, i_image: int | None = None
    ...
    elements = [atom.GetSymbol() for atom in mol_r.GetAtoms()]  # type: ignore
    ...
    path = interpolate_geodesic(elements, [coordinates_r, coordinates_p_ts], n_images=n_images)
    if i_image is None:
        i_image = n_images // 2
    ...
    coordinates_ts_crest = conformer_ensemble_ts[0].coordinates  # type: ignore
    ...
    energy_reaction = results.shift_results.energy_diff_gfn * HARTREE_TO_KCAL  # type: ignore

    energy_reactant = results.shift_results.energies_ff[0]  # type: ignore
...
def substructure_preserver(mol):
    ...
    mol = AddHs(mol)  # Note: Hydrogens need to be added for the substructure code to work!

    if (
        mol.HasSubstructMatch(
            MolFromSmarts(
                "[H][C@@]1(*)[C@;R2](*)2[C@@]34[C@;R2]5(*)[C;R1](*)=[C;R1](*)[C@;R2](*)([*;R2]5)[C@@]3([*;R1]4)[C@](*)([*;R2]2)[C@@;R1]1([*])[H]"
            )
        )
        is True
    ):
...
def substructure_violations(mol):
    ...
    forbidden_fragments = [
        ...
        "[S&X6]",
        "[P,p]",
        "[B,b,N,n,O,o,S,s]~[F,Cl,Br,I]",
        "*=*=*",
        "*#*",
        "[O,o,S,s]~[O,o,S,s]",
        "[N,n,O,o,S,s]~[N,n,O,o,S,s]~[N,n,O,o,S,s]",
        "[N,n,O,o,S,s]~[N,n,O,o,S,s]~[C,c]=,:[O,o,S,s,N,n;!R]",
        "*=N-[*;!R]",
        "*~[N,n,O,o,S,s]-[N,n,O,o,S,s;!R]",
    ]
    for ni in range(len(forbidden_fragments)):
        if mol.HasSubstructMatch(Chem.MolFromSmarts(forbidden_fragments[ni])) is True:
            violation = True
            break
        else:
            continue
...
def get_reactivity(smi: str, verbose: bool = False, n_procs: int = 1):
    ...
    if substr_vio is True or call_ is False:
        # return -10**4, -10**4, 10**4
        return 10**4, 10**4, 10**4, 10**4

    # Normal calculation can procedd (no fitness violations):
    smiles_stereo, activation_energy, reaction_energy, sa_score, run_time = run_reaction(
        smi, n_procs=n_procs
    )
...
if __name__ == "__main__":
    ...
    Ea, Er, sum_Ea_Er, diff_Ea_Er = get_reactivity(smi)
